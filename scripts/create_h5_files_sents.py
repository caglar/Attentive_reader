...
class DescIndex(tables.IsDescription):
    posw = tables.UInt32Col()
    poss = tables.UInt32Col()
    dlength = tables.UInt32Col()
    slength = tables.Uint32Col()
...
indices = f.createTable("/", 'desc_indices',
    DescIndex, "a table of indices for descs and lengths")
...
def add_sentence_info(d_ind, pos_wdesc, descs):
    sent_pos = []
    sent_lens = []
    sent_pos.append(pos_wdesc)
    slen = 0
    for i, dw in enumerate(descs):
        if dw == desc_vocab["."] or dw == desc_vocab["!"] or dw == desc_vocab["?"]:
            if i != len(descs) - 1:
                sent_pos.append(pos_wdesc + i + 1)
                slen += 1
                if not (i == len(desc) - 2  and tok == desc_vocab['EOS']):
...
for root, dir_, files in os.walk(args.input_dir):
    for file_ in files:
        ...
        d_ind = indices.row
        d_ind['posw'] = pos_wdesc
        d_ind['dlength'] = len(d_ids)
        add_sentence_info(d_ind, pos_wdesc, d_ids)
        d_ind.append()
        q_ind = qs_ids.row
        q_ind['pos'] = pos_q
